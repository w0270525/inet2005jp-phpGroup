      ...
      <div class="container-fluid">
          ...
          <div class="btn-group">
              ...
              <form action="#" method="post" name="logout" id="logout" style="display:inline;" >
                  <input type="submit" name="logout" id="logout" class="btn btn-primary pull-right"
                         value="logout" style="display:inline;right:0;" />
              </form>
              <!-- clears the current for 100% -->
              <form action="index.php" method="post" name="" id="resetButton" style="display:inline;" >
                  <input type="submit" name="" id="" class="btn btn-success pull-right"
                         value="Reset" style="display:inline;right:0;" />
              </form>
...
?>
      ...
      <div class="btn-group" id="btnGroup Editor">
        ...
        <!-- hide the Author menus if user is more then Author  -->
<?php if(CMS_checkAuthor()):?>

      <form action="" method="post" name="" id="HideAuthorNav" style="display:inline;" >
          <input type="submit" name="HideAuthor" id="HideAuthor" class="btn btn-success pull-right"
                 value="Hide Author Buttons" style="display:inline;right:0;" />
      </form>
...
?>
    ...
    <?php
        ...
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            if((isset($_GET["update"]) && (isset($_POST["updateId"])
                  && checkVar($_POST["updateId"]) && checkVar($_POST["userName"])
                  && checkVar($_POST["FirstName"]) && checkVar($_POST["LastName"])
                  && checkVar($_POST["Createdby"])) && $_GET["update"] == $_POST["updateId"]))
            {
                $tempController->userController()->commitUpdateAction($_POST["updateId"],
                             $_POST["userName"], $_POST["FirstName"], $_POST["LastName"],
                             $roles,$_POST["Createdby"],$control->currentUser->getId());
            }

            if(isset($_POST["userName"]) && (isset($_POST["bnasd3432er"])
                  && checkVar($_POST["userName"]) && checkVar($_POST["FirstName"]) && checkVar($_POST["LastName"])))
            {
    ...
    ?>
        ...
        <!--CUSTOM ADMIN VIEWS -->
        <!--CUSTOM ADMIN VIEWS -->
        <div id="viewUsers" class="containerAdmin">
          <?php $tempController->userController()->displayAction(); ?>
        </div>

        <div id="addNUser" class="containerAdmin">
          <?php $tempController->userController()->addNewUser(); ?>
        </div>
    ...
    if(CMS_checkEditor())
    {
        ...
        // CALLS TO PROCESS THE UPDATE FORMS
        if($_SERVER["REQUEST_METHOD"]=="POST")
        {
            ...
            if(isset($_POST["formSubmitEditContentArea"]) && $_POST["formSubmitEditContentArea"]=="true"){
                $control->contentController()->confirmUpdateAction( $_POST["c_name"],$_POST["c_alias"],
                                                                    $_POST["c_desc"],$_POST["c_order"]);
            }
            ...
            // process new article area form
            if(isset ($_POST["a_name"]) && isset($_POST["a_title"]) && isset($_POST["a_desc"])
              && isset($_POST["formSubmitNewArticleConfirm"]) && $_POST["formSubmitNewArticleConfirm"] == true)
            {
            ...
            if(isset($_POST["formUpdateStyle"]) &&  $_POST["formUpdateStyle"]=="true"){
                $control->styleController()->updateActionConfirm($_POST["s_id"],$_POST["s_name"],
                                                                 $_POST['s_desc'], $_POST['s_style']);
            }
            ...
            if(isset($_POST["formEditArticleConfirm"]) && $_POST["formEditArticleConfirm"]=="true")
            {
                ...
                $control->articleController()->updateActionConfirm($_POST['a_id'], $_POST['a_name'],
                          $_POST['a_title'], $_POST['a_desc'],$_POST['a_blurb'],$_POST['a_content'],
                          $_POST['all_page'],$_POST['a_contentarea'],$_POST['a_page'],$_POST["a_inactive"]);
            }
